...
def test_sync_all_emits_identity_header_first(tmp_path: Path):
    ...
    # The sync entry wires the shared loud exit guard (F-08) on its
    # success path; the guard's own contract lives in test_logging.py.
    assert "guard_process_exit" in sync.main.__code__.co_names
