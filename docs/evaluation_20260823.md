...
17. 最近一期协议筛选（45 试错）最佳候选是 `baseline:ILLIQ_20` 而非 RL 公式，
    DSR 0.35 / max-t p 0.74 均不显著：RL 搜索尚未产生可辩护的 alpha。建议下一步
    不是加大训练时长，而是：a) 修完 P0 因果问题后重跑协议基线；b) 用
    `ablate_families.py` 找真正出力的因子族，收缩搜索空间；c) 把随机搜索基线当作
    下界，先让 RL 稳定 ≥ 随机搜索再谈升档。
