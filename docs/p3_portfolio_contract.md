...
`RebalancePolicy` 接受六个规范值（P6 修订：新增 `every_20_days` 与
`monthly`，`REBALANCE_POLICY_VERSION` 1→2；契约条款
`docs/p6_research_domain_contract.md` §2）：
...
| `every_20_days` | 从数据集首个交易日起按全局交易日序号 `0, 20, 40, ...` |
| `monthly` | 完整日期轴上每个自然月的最后一个交易日（P6 新增） |
...
允许 1～5；`every_10_days` 允许 1～10；`every_20_days` 允许 1～20；`monthly`
无静态上限，但 `horizon > 1` 时在完整日期轴上校验相邻月度信号间隔 ≥ horizon
（真实 A 股日历的春节二月只有约 15 个交易日，`(monthly, h≥20)` 在多数日期轴
上不可执行，必须 fail-fast，不得静默抽稀或重叠）。无效组合必须在配置阶段拒绝，
不能静默抽稀、重叠或改写 frequency。多周期 target 的非调仓列为 `NaN`；学习、
验证和 OOS 统计只消费调仓列。这个约束保证任意两个有效标签的开区间收益段不
重叠，边界 open 可以相接但不共享收益。

每个验证子窗口必须至少包含两个全局调仓观察，才能识别跨日期 ICIR；不足时必须在
候选评分前明确拒绝，不能以中性分数静默继续。fold 的 frequency/horizon 必须与
执行它的 BacktestConfig 完全一致，语义缓存窗口标识也必须包含这两个字段。

`AshareDataLoader.target_ret` 是为旧调用保留的相邻 open 日收益字段，不是 P3 的
稀疏多周期研究标签。正式 P3 训练、搜索、诊断与评估路径必须从完整日期轴的
`RebalancePolicy` 单独构造因果 target；旧字段只能按 daily/horizon=1 语义读取。
...
7. 缩放后再次执行最小交易金额过滤；过滤可能把减仓恢复为旧权重，因此量化前
   必须再次执行现金上限，只同比缩小仍存在的新买入，保证总权重不超过 1，绝不
   向上归一化任何持仓；最后量化到 1e-12。
...
- `P3_MEASUREMENT_VERSION`: 1 -> 2 -> 3；v1 未记录 reward/backtest 逐日订单数差，
  v2 的 `every_N_days` 审计从样本窗重新锚定；两者都不能作为最终 P3 验收证据。
  v3 同时记录完整日期轴索引和样本相对索引；旧测量必须用当前入口重新运行，不能
  手工补字段。
...
标准复现入口为 `python -m ashare_model.p3_measurement`，默认写入
`data/p3_measurement.json`；报告必须同时记录数据集、抽样规则、完整股票代码、
日期窗、配置、seed、代码提交和运行环境，且不得把该诊断解释为 Alpha 证据。
