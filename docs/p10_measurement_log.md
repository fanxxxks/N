...
# Stage B/C：OOS 评价、回测硬门槛与预注册裁决（t8，2026-09-01）

## 10. 运行身份

| 项 | 值 |
|---|---|
| 命令 | `python -m ashare_model.searcher_adjudication --run-dir data/p10_searcher_comparison --fold 0` |
| 实现 commit | `5321219`（searcher_adjudication.py，P10_ADJUDICATION_VERSION=1；RED：测试文件 collection ImportError 先行记录；顺带修掉两个测试侧错误——area 阶梯约定用例期望与 fixture 形状，实现语义未变） |
| 被测 campaign | run_id `a45764aadaab444b9b0fa4a1c60b35a1`（status=completed 校验通过，非 completed 拒绝裁决——fail-closed） |
| 评价路径 | `evaluation.evaluate_formula`（全历史执行 → fold-0 测试窗 (2020-12-31, 2021-12-31] 切片 → `evaluate_signal` 全成本引擎），direction = 搜索时 `selected.direction`（12/12 行捕获验证，无重拟合路径） |
| dataset_id | `a839ecf2…`（与 Stage A 一致） |
| 耗时 | 409.9s（12 公式 + 等权基准行）；production gates 入口通过 |
| 产物 | `data/p10_searcher_comparison/adjudication.json`（新文件，原子写；原始未跟踪） |

## 11. 逐公式 OOS 结果与硬门槛判定（§5.2：四条全过才算过）

| seed:searcher | 年化 | 最大回撤 | Sharpe | Calmar | 日均换手 | 实际调仓/单/抑制 | 超额收益 | 测试窗 icir | 方向 | 门槛 |
|---|---|---|---|---|---|---|---|---|---|---|
| 42:gp | 0.72% | 20.51% | −0.068 | 0.035 | 0.68% | 6/33/7588 | −11.0% | 0.008 | +1 | ✗ |
| 42:tpe | 12.25% | 12.20% | 0.571 | 1.004 | 0.44% | 2/21/8028 | −1.28% | 0.035 | −1 | ✗ |
| 42:random | −4.01% | 18.35% | −0.266 | −0.218 | 0.44% | 2/21/5940 | −15.0% | 0.125 | +1 | ✗ |
| 42:rl | 12.97% | 12.89% | 0.668 | 1.006 | 0.46% | 3/22/9064 | −0.68% | −0.019 | −1 | ✗ |
| 7:gp | 16.02% | 12.15% | 0.768 | 1.318 | 0.44% | 2/21/8058 | +1.88% | 0.137 | −1 | ✗ |
| 7:tpe | 14.86% | 11.38% | 0.823 | 1.307 | 0.56% | 6/27/7954 | +0.92% | 0.063 | +1 | ✗ |
| 7:random | **22.77%** | **7.52%** | **1.268** | **3.027** | 0.50% | 5/24/8640 | **+7.55%** | 0.043 | −1 | **✓ 全过** |
| 7:rl | −4.80% | 12.75% | −0.413 | −0.377 | 0.46% | 3/22/8849 | −15.68% | 0.207 | +1 | ✗ |
| 2024:gp | 10.43% | 11.26% | 0.466 | 0.926 | 0.48% | 3/23/7609 | −2.82% | 0.132 | −1 | ✗ |
| 2024:tpe | 10.80% | 8.93% | 0.516 | 1.209 | 0.48% | 4/23/7809 | −2.51% | −0.029 | −1 | ✗ |
| 2024:random | −3.04% | 16.81% | −0.254 | −0.181 | 0.54% | 4/26/8566 | −14.18% | 0.077 | −1 | ✗ |
| 2024:rl | 16.36% | 20.71% | 0.548 | 0.790 | 0.44% | 2/21/8063 | +2.17% | 0.152 | −1 | ✗ |
| 等权基准 | 13.77% | 7.58% | 0.784 | — | — | — | — | — | — | — |

逐行事实：

- **唯一全过四门槛的公式**：random seed-7（年化 22.77% > 10%、回撤 7.52% < 15%、Sharpe 1.268 ≥ 1.0、Calmar 3.027 ≥ 1.0，超额 +7.55%）。但**组合活动门披露**：全年仅 5 次实际调仓、24 笔订单、8640 笔被抑制（日调仓 schedule 下的近静态持仓）——按 AGENTS §8.5，此类低活动"达标"不得单独解读为有效（t8 裁决在臂级执行，单 seed 不构成臂级通过）。
- **组合活动门全面揭示**：12 行全部为 2–6 次实际调仓/年、21–33 笔订单、5940–9064 笔抑制交易、日均换手 0.44–0.68%——搜索选出的公式在日调仓协议下几乎都是近静态慢信号书。这直接印证契约 §5.2 活动门披露的必要性与 t2 的成本-换手结构发现。
- **跑输基准为主**：12 行中 8 行超额收益为负（测试窗 (2020-12-31, 2021-12-31] 等权基准年化 13.77%）。
- seed 间方差极大（同一臂年化 −4.8% → +16.4%），n=3 仅支持方向性描述，无统计显著性声明（契约 §5.3）。
- `oos_positive` 的执行口径说明：契约 §5.3 写作 "active_ir > 0"，本裁决以单一路径 payload 的测试窗 `icir > 0` 执行（不新建第二套 IR 语义；搜索期 active_ir 保留在 campaign 产物中作上下文）。

## 12. 臂级聚合与 §8 预注册裁决执行

| 臂 | area 中位数（B=2000 积分） | admissible（≥2/3 全过） | oos_positive（≥2/3 icir>0） | stagnant（全 seed 停滞且消耗 <B/2） |
|---|---|---|---|---|
| gp | 1959.5 | ✗（0/3） | ✓ | ✓（唯一停滞臂） |
| tpe | 1952.4 | ✗（0/3） | ✓ | ✗ |
| random | 1951.9 | ✗（1/3） | ✓ | ✗ |
...
- **裁决 headline = `negative_no_admissible_formula`**（匹配行：`no_arm_admissible_negative_result`；无 additional findings——"全臂停滞"不成立，efficient 臂 OOS 为正）。
- **§8 预注册裁决原文执行**："无任何臂 backtest_admissible → 合法负结果：当前预算/窗口/成本下无搜索器产出可过门槛公式；方向转向评价器/Reward 与成本-换手结构（引用 t2 成本证据），不归咎某一搜索器。"
- **area 趋同事实**：四臂 area 中位数差异 < 0.4%（1951.6–1959.5）——Stage A 的 best-so-far 曲线全部在极早期到达 0.98 平台（终点 12/12 一致），area 几乎不区分臂。"efficient 臂 = gp"按预注册定义成立但属边际差异，在 n=3 与 area 趋同事实下不构成单赢声明。
- 门槛边界实例：42:rl Calmar 1.006（≥1.0 过）但年化 12.97% 过、Sharpe 0.668 不过；42:tpe Calmar 1.004 过、Sharpe 0.571 不过——单条门槛的通过不构成臂级通过。

## 13. Stage B/C 验证与研究边界

- 被测实现 commit `5321219` 的验证：test_p10_adjudication 6/6（含门槛边界严格性、direction 不可重拟合捕获测试、area 阶梯约定、§8 五情形合成用例）；test_p10_campaign + test_eval_module_split 18 passed；evaluate_signal 消费方（test_evaluation / test_stitched_oos / test_admission / test_baseline_harness）88 passed（additive 活动门字段无回归）；compileall 0 错；git diff --check 干净；分片 union 90 文件。
- 提交：`5321219`（adjudicator 实现，4 文件 +750）+ 本日志的证据 commit。
- **研究边界（负结果为合法结论）**：本次裁决的结论是"在 B=2000 预算、fold-0 窗口与全成本日调仓协议下，无搜索器的选中公式族达到臂级回测门槛"。它**不证明**任何搜索器"更差"，不构成对生产默认搜索器（gp）的变更依据，不构成 alpha/晋级/lifecycle 结论；paper/sim 模拟盘操作明确不在本任务范围（t8 标题中的"模拟盘达标检验"以回测硬门槛 + 组合活动门检验执行，实际 paper 观察属 P8 lifecycle 另行预注册）。改进方向按 §8 映射指向评价器/Reward 与成本-换手结构（含稀疏覆盖、奖励饱和 0.98 平台的构造、近静态持仓的活动门设计），由 captain 立项。
