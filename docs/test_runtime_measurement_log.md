...
### U9 裁决追加（2026-09-03，选项 a：.venv 定向降级 scs）

- 授权：用户裁决采纳选项 a——.venv 定向降级 `scs==3.2.11`（其余 11+1
  包对齐全保留，torch 不动）；lock 侧 `scs==3.3.0` pin 缺陷修复（需
  重生成 lock = 写盘操作）另立任务 t22，属用户另行授权范围，本记录
  仅覆盖 .venv 侧。上文"按 b) 现状落账"由本节取代（append-only，
  不改写已发生记录）。
- 命令：`pip install "scs==3.2.11"`（.venv 解释器）→ exit 0，实装
  3.2.11；pip 降级残留临时目录 `~cs`/`~cs.libs` 已随事务清理。
- 复验（与裁决预期逐项对应）：
  1. `scripts/lock_audit.py` → **恰 1 条 mismatch**：
     `scs: lock=3.3.0 env=3.2.11 [lock_newer]`——即唯一已裁决豁免项；
     其余 104 包零漂移，torch 本地 tag 说明照旧（P0-06 设计内）。
     exit 1 为工具契约内的"存在漂移"信号，语义 = 登记在案的豁免。
  2. `.venv pip check` → exit 0。
  3. 最小 LP 求解冒烟（隔离子进程，SCS v3.2.11）→ **正常退出 0**，
     求解器完整迭代并出报告（3.3.0 的硬崩消除）。冒烟用 toy 问题本身
     无界（status "unbounded"）为脚本构造属性，非求解器缺陷。
- 现存 `--check-full` 残差（均为已裁决设计态，非漂移缺陷）：
  ①torch 本地 tag（lock +cpu vs 实装 +cu128，P0-06）；②scs 豁免
